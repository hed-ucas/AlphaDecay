...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model(
            input_ids=input_ids, #维度为[64,256]
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        hidden_states = outputs[0] #维度为torch.Size([64, 256, 512])，就是对其进行了向量化
        logits = self.lm_head(hidden_states) #self.lm_head为一个Linear(in_features=512, out_features=32000, bias=False)，也就是预测得到进行向量化以后，验证能不能还原为正确的整数编码
