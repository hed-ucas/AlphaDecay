...
def train_one_epoch(args, model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    num_training_steps_per_epoch=None, update_freq=None, use_amp=False, wd_scheduler=None, rank=None):
    model.train(True)
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if (args.use_modulewise_wd) and use_AlphaDecay == True:
            gradnorm = calculate_layer_gradnorms(model.module if hasattr(model, 'module') else model)
            print('----> One step of Temp Balancing')
            layermean_alpha = wd_scheduler.step(optimizer, wd, it, rank0=rank==0, gradnorm=gradnorm)
            use_AlphaDecay = False
            # print(optimizer)
        ...
        if wandb_logger:
            wandb_logger._wandb.log({
                ...
                'Rank-0 Batch Wise/class_acc': class_acc, 
